...
func Auth() gin.HandlerFunc {
	...
	return func(c *gin.Context) {
		...
		if tokenStr == "" {
			...
			// 读取令牌并顺带续期，一次往返完成。
			// 读取失败要分清"令牌确实不在"与"这次没读到"：后者是缓存后端故障，与登录状态无关，
			// 据此要求重新登录会把一次后端抖动变成一次强制下线。
			var tokenInfo model.TokenInfo
			tokenErr := global.GCACHE_WAFCACHE.GetAsEx(enums.CACHE_TOKEN+tokenStr, &tokenInfo,
				time.Duration(global.GCONFIG_RECORD_TOKEN_EXPIRE_MINTUTES)*time.Minute)
			if tokenErr != nil {
				// 令牌只打前 8 位，避免把可用凭证写进日志。
				shortToken := utils.TruncateString(tokenStr, 8)
				switch {
				case errors.Is(tokenErr, cache.ErrCacheMiss):
					zlog.Debug(fmt.Sprintf("令牌不在缓存中(已过期/已注销/被同账号新登录顶掉) token:%v... path:%v 来源IP:%v",
						shortToken, c.Request.URL.Path, utils.GetManageClientIP(c)))
					response.AuthFailWithMessage("令牌过期", c)
				case errors.Is(tokenErr, cache.ErrCacheBackend):
					zlog.Error(fmt.Sprintf("令牌读取失败，缓存后端不可用 token:%v... path:%v err:%v",
						shortToken, c.Request.URL.Path, tokenErr))
					response.BackendUnavailableWithMessage("服务暂时不可用，请稍后重试", c)
				default:
					zlog.Error(fmt.Sprintf("令牌缓存内容解析失败 token:%v... err:%v", shortToken, tokenErr))
					response.AuthFailWithMessage("令牌数据异常，请重新登录", c)
				}
				c.Abort()
				return
			}

			// 缓存内容必须是一个完整会话：账号为空说明取到的数据不完整，按无效处理。
			if tokenInfo.LoginAccount == "" {
				zlog.Error(fmt.Sprintf("令牌缓存内容不完整(账号为空) token:%v...", utils.TruncateString(tokenStr, 8)))
				response.AuthFailWithMessage("令牌数据异常，请重新登录", c)
				c.Abort()
				return
			}
			...
			// 开发构建下打印令牌缓存剩余时间；续期本身已在上面的 GetAsEx 里完成
...
// 指纹与严格IP绑定都关闭时不存在该计数，整段跳过——这是每个请求都会走到的路径，
// 缓存后端为 Redis 时一次跳过就少一次往返。
func clearTokenBindFailure(tokenStr string) {
	if global.GCONFIG_ENABLE_STRICT_IP_BINDING != 1 && global.GCONFIG_ENABLE_DEVICE_FINGERPRINT != 1 {
		return
	}
	global.GCACHE_WAFCACHE.Remove(enums.CACHE_TOKEN_BINDFAIL + tokenStr)
}
